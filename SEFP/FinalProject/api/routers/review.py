...
@router.get("/low-rated/{max_rating}", response_model=list[schema.Review])
def low_rated_reviews(max_rating: int, db: Session = Depends(get_db)):
    return controller.low_rated_reviews(db, max_rating)